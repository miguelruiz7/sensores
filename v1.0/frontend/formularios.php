...
<form id="agregarSeccion">
 ...
 <h5 class="offcanvas-title text-light" id="offcanvasExampleLabel">Usuarios</h5>
   <button type="button" class="btn text-light"  data-bs-dismiss="modal" aria-label="Close"data-bs-dismiss="modal" aria-label="Close" onclick="revertirFormulario()">Cerrar</button>
          ...
          $id = $_POST['id']; 
          $consulta = "SELECT * FROM esp_det, usr_mst, usrol_mst WHERE esp_esp_id = $id AND esp_usr_id = usr_id AND esp_usrol_id = usrol_id ORDER BY usrol_id ASC;";
          $buscaUsuarios = mysqli_query($conexion, $consulta);

          if(mysqli_num_rows($buscaUsuarios)>0){

              #Antes de que nos muestre la información únicamente la podra ver el administrador
              $detectaRolUsr = detectarRolUsuario($sesion, $conexion);
              if($detectaRolUsr == (2) && $detectaRolUsr != 4 ){
            ?>
           ...
           while($datosUsuarios = mysqli_fetch_array($buscaUsuarios)){
            ...
            ?>
             <tr>
                ...
                <?php if($detectaRolUsr != 3) { ?>  <td><?php echo $datosUsuarios['usr_usu'];?></td>
...
<form id="modificaUsuario">
          ...
          <div class="form-floating text-light mb-3">
              ...
              <option value="<?php echo $datosUsuario['esp_usrol_id'];?>" selected><?php echo $datosUsuario['usrol_nom'];?></option> 
                      <?php
                      # Queda pendiente realizar el código mientras que no evaluen el diagrama de entidad relación
                           $id_tipo_rol = $datosUsuario['esp_usrol_id'];
                             $consulta="SELECT * FROM usrol_mst WHERE usrol_id NOT IN ('$id_tipo_rol')";
                              $buscarolesEspacio= mysqli_query($conexion,$consulta);
                                  while($valores= mysqli_fetch_array($buscarolesEspacio)){
                                      ?>
                                      <option class="bg-none" value="<?php echo $valores['usrol_id'];?>" style="background-color:#042f52; filter: blur(5px);"><?php echo $valores['usrol_nom']; ?></option>
                                      <?php
                                  }       
                      ?>
                 </select>
