...
<form id="agregarEspacio">
          ...
          <div class="form-floating text-light mb-3">
            <select id="txt_esp_espt_id" class="form-control border-bottom border-0 border-bottom-2 border-light bg-transparent rounded-0 text-white" id="floatingInput" >
                    ...
                    # Queda pendiente realizar el código mientras que no evaluen el diagrama de entidad relación
                           ...
                           $consulta="SELECT * FROM esp_tipo_mst";
                            $buscatiposEspacio= mysqli_query($conexion,$consulta);
                                while($valores= mysqli_fetch_array($buscatiposEspacio)){
                                    ?>
                                    <option class="bg-none" value="<?php echo $valores['esp_tipo_id'];?>" style="background-color:#042f52; filter: blur(5px);"><?php echo $valores['esp_tipo_nom']; ?></option>
                                    <?php
                                } 
        ...
        case 'modificarEspacio':
          ...
          $consulta = "SELECT * FROM esp_mst, esp_tipo_mst WHERE esp_id='$id' AND esp_esp_tipo_id = esp_tipo_id";
  ...
  <form id="modificaEspacio">
            ...
            <div class="form-floating text-light mb-3">
              <select id="txt_esp_espt_id" class="form-control border-bottom border-0 border-bottom-2 border-light bg-transparent rounded-0 text-white" id="floatingInput" >
              <option value="<?php echo $datosEspacio['esp_tipo_id'];?>" selected><?php echo $datosEspacio['esp_tipo_nom'];?></option> 
                      <?php
                      # Queda pendiente realizar el código mientras que no evaluen el diagrama de entidad relación
                           $id_tipo_esp = $datosEspacio['esp_esp_tipo_id'];
                             $consulta="SELECT * FROM esp_tipo_mst WHERE esp_tipo_id NOT IN ('$id_tipo_esp')";
                              $buscatiposEspacio= mysqli_query($conexion,$consulta);
                                  while($valores= mysqli_fetch_array($buscatiposEspacio)){
                                      ?>
                                      <option class="bg-none" value="<?php echo $valores['esp_tipo_id'];?>" style="background-color:#042f52; filter: blur(5px);"><?php echo $valores['esp_tipo_nom']; ?></option>
...
<form id="modificaUsuario">
        ...
        case 'asignausuarioEspacio': 
          $espacio = $_POST['espacio'];
        ?>
         <div class="offcanvas-header m-3">
 <h5 class="offcanvas-title text-light" id="offcanvasExampleLabel">Asignar usuario</h5>
   <button type="button" class="btn text-light"  onclick="revertirFormulario(); formUsuariosEsp(<?php echo $espacio; ?>)">Atras</button>
 </div>
<form id="asignaUsuario">

        <div class="form-floating text-light mb-3">
                      <select id="txt_usr_nom" class="form-control border-bottom border-0 border-bottom-2 border-light bg-transparent rounded-0 text-white" id="floatingInput" >
          ...
          <div class="form-floating text-light mb-3">
              ...
              <option value="" selected></option> 
                      ...
                      # Queda pendiente realizar el código mientras que no evaluen el diagrama de entidad relación
                           $id_tipo_rol = $datosUsuario['esp_usrol_id'];
                             $consulta="SELECT * FROM usrol_mst WHERE usrol_id AND usrol_id NOT IN ('1') AND usrol_id NOT IN ('2')";
                              $buscarolesEspacio= mysqli_query($conexion,$consulta);
                                  while($valores= mysqli_fetch_array($buscarolesEspacio)){
                                      ?>
                                      <option class="bg-none" value="<?php echo $valores['usrol_id'];?>" style="background-color:#042f52; filter: blur(5px);"><?php echo $valores['usrol_nom'].' ('.$valores['usrol_desc'].')'; ?></option>
              ...
              <label>Seleciona el rol</label>
            </div>


          <div class="text-center">
          <button type="button" id="funcion" value="asignarUsuario" onclick="asignarUsuario('<?php echo $espacio;?>')" class="btn btn-outline-light">Aceptar</button>
          </div> 
